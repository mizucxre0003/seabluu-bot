...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    hello = (
        "✨ Привет! Я *SEABLUU* Helper"
        "Помогаю отслеживать разборы и доставку, храню адрес и присылаю уведомления"
        "*Что умею:*"
        "🔍 Отследить разбор — покажу статус по order_id (например, CN-12345)."
        "🔔 Подписки — уведомлю, когда статус заказа изменится."
        "🏠 Мои адреса — сохраню/обновлю адрес для доставки."
        "Если что-то пошло не так нажми на <отмена> или пропиши /help"
    )
