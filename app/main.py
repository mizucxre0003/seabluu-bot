...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Общий текстовый роутер для клиентов (админов перехватит on_admin_text c group=-10)."""
...
async def save_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Пример: данные берите из вашей формы
    ...
    # Автоподписка — если используется в вашем проекте
    try:
        rel_orders: List[str] = sheets.find_orders_for_username(username.lower())
        # for oid in rel_orders: sheets.subscribe(u.id, oid)  # если есть
    except Exception:
        pass
...
async def remind_unpaid_for_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Просим ввести order_id; реальная рассылка — в on_text_after_remind."""
    if not _is_admin(update.effective_user.id):
        return
    if update.message:
        await update.message.reply_text("Введи order_id для рассылки неплательщикам:")
    else:
        await context.bot.send_message(update.effective_chat.id, "Введи order_id для рассылки неплательщикам:")
...
async def on_text_after_remind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик ввода order_id для рассылки по одному разбору + отчёт админам."""
    if not update.message:
        return
...
async def broadcast_all_unpaid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Массовая рассылка по всем разборам + сводка."""
    if not _is_admin(update.effective_user.id):
        return
...
async def on_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Роутер для админских текстов. Регистрируется с group=-10, чтобы обгонять on_text."""
    if not update.message:
        return
    if not _is_admin(update.effective_user.id):
        return
    ...
    if text == "уведомления всем должникам":
        await broadcast_all_unpaid(update, context)
        return

    if text == "уведомления по id разбора":
        await remind_unpaid_for_order(update, context)
        return
...
def register_handlers(application):
    """Зарегистрируйте здесь ваши стандартные хэндлеры (как в исходном проекте)."""
    application.add_handler(CommandHandler("start", start))
    # общий текст (клиентский)
    application.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), on_text))
    # ввод order_id после запроса на рассылку по одному разбору
...
def register_admin_ui(application):
    """Регистрируем админ-UI с приоритетом group=-10, чтобы он срабатывал раньше on_text."""
...
# Здесь можно добавить ваши дополнительные CallbackQueryHandler и пр.
